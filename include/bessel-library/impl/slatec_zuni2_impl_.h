/*
  ...
  Description: Slatec's ZUNI2 subroutine, originally translated from Fortran to
...
L10:
  ...
  /* ----------------------------------------------------------------------- */
  fn = fmax_impl_(*fnu, 1.);
  slatec_zunhj_impl_(&znr, &zni, &fn, &c_u_1, tol, &phir, &phii, &argr, &argi,
...
L40:
  ...
  for (i_u_ = 1; i_u_ <= i_u_1; ++i_u_) {
    ...
    slatec_zunhj_impl_(&znr, &zni, &fn, &c_u_0, tol, &phir, &phii, &argr, &argi,
                       &zeta1r, &zeta1i, &zeta2r, &zeta2i, &asumr, &asumi,
                       &bsumr, &bsumi);
    if (*kode == 1) {
      goto L50;
    }
  ...
  L70:
    ...
    slatec_zairy_impl_(&argr, &argi, &c_u_1, &c_u_2, &dair, &daii, &ndai,
                       &idum);
