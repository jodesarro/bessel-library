/*
  ...
  Description: Implements and computes a n-sequency array, in double complex
  type for C, or in std::complex<double> type for C++, of cylindrical Hankel
...
/*
  Implements and computes a n-sequency array of cylindrical Hankel functions of
  the second kind, real order nu, and complex argument z, i.e., {H2_nu(z),
  H2_(nu+1)(z), ..., H2_(nu+n-1)(z)}, for also negative orders, by means of the
  ...
  abs(z)=0.
*/
static inline void cyl_h2_full_seq_impl_(double nu, int n, dcomplex z,
                                         dcomplex *cyl_h2_arr, int scaled) {
  ...
  if (cabs(z) < DBL_EPSILON) {
    ...
    /* Compute zbesh */
    slatec_zbesh_impl_(&x, &y, &fnu, &kode, &m, &n, &ch2r[0], &ch2i[0], &nz,
                       &ierr);
    slatec_flags_zbesh_impl_(ierr, nz);

    /* Store in the array */
    for (int i = 0; i < n; i++) {
      cyl_h2_arr[i] = ch2r[i] + I * ch2i[i];
    }
    ...
    slatec_zbesh_impl_(&x, &y, &fnu_m, &kode, &m, &n, &ch2r_m[0], &ch2i_m[0],
                       &nz, &ierr);
    slatec_flags_zbesh_impl_(ierr, nz);
    ...
    for (int i = 0; i < n; i++) {
      ...
      /* Eq. (9.1.6) of Ref. [1] */
      cyl_h2_arr[i] = (ch2r_m[tmp1] + I * ch2i_m[tmp1]) * cexp(-I * tmp2);
    }
    ...
    /* Compute zbesh */
    slatec_zbesh_impl_(&x, &y, &fnu_m, &kode, &m, &n_m, &ch2r_m[0], &ch2i_m[0],
                       &nz, &ierr);
    slatec_flags_zbesh_impl_(ierr, nz);
    ...
    slatec_zbesh_impl_(&x, &y, &fnu_p, &kode, &m, &n_p, &ch2r_p[0], &ch2i_p[0],
                       &nz, &ierr);
    slatec_flags_zbesh_impl_(ierr, nz);
    ...
    for (int i = n_m; i < n; i++) {
      int tmp1 = i - n_m;
      cyl_h2_arr[i] = ch2r_p[tmp1] + I * ch2i_p[tmp1];
    }
