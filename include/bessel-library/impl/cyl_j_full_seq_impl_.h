/*
  ...
  for C, or in std::complex<double> for C++, of cylindrical Bessel functions of
  the first kind, real order nu, and complex argument z, i.e., {J_nu(z),
  J_(nu+1)(z), ..., J_(nu+n-1)(z)}, for also negative orders, by means of the
...
/*
  ...
  abs(z)=0.
*/
static inline void cyl_j_full_seq_impl_(double nu, int n, dcomplex z,
                                        dcomplex *cyl_j_arr, int scaled) {
  ...
  if (nu >= 0.0) {
    ...
    /* Compute zbesj */
    slatec_zbesj_impl_(&x, &y, &fnu, &kode, &n, &cjr_ptr[0], &cji_ptr[0], &nz,
                       &ierr);
    slatec_flags_zbesj_impl_(ierr, nz);

    /* Store in the array */
    for (int i = 0; i < n; i++) {
      cyl_j_arr[i] = cjr_ptr[i] + I * cji_ptr[i];
    }
    ...
    if (fabs(floor(fnu) - fnu) < DBL_EPSILON) {
      ...
      for (int i = 0; i < n; i++) {
        int tmp = n - 1 - i;
        cyl_j_arr[i] =
            pow(-1.0, fnu_m + tmp) * (cjr_ptr[tmp] + I * cji_ptr[tmp]);
      }
      ...
      for (int i = 0; i < n; i++) {
        ...
        /* Eq. (5.5.4) of Ref. [2] */
        cyl_j_arr[i] = (cjr_ptr[tmp1] + I * cji_ptr[tmp1]) * cos(tmp2) -
                       (cyr_ptr[tmp1] + I * cyi_ptr[tmp1]) * sin(tmp2);
      }
    ...
    if (fabs(floor(fnu) - fnu) < DBL_EPSILON) {
      ...
      for (int i = 0; i < n_m; i++) {
        int tmp = n_m - 1 - i;
        cyl_j_arr[i] =
            pow(-1.0, fnu_m + tmp) * (cjr_m_ptr[tmp] + I * cji_m_ptr[tmp]);
      }
      ...
      for (int i = 0; i < n_m; i++) {
        ...
        double tmp2 = (fnu_m + (double)tmp1) * M_PI;
        /* Eq. (5.5.4) of Ref. [2] */
        cyl_j_arr[i] = (cjr_m_ptr[tmp1] + I * cji_m_ptr[tmp1]) * cos(tmp2) -
                       (cyr_m_ptr[tmp1] + I * cyi_m_ptr[tmp1]) * sin(tmp2);
      }
    ...
    for (int i = n_m; i < n; i++) {
      int tmp1 = i - n_m;
      cyl_j_arr[i] = cjr_p_ptr[tmp1] + I * cji_p_ptr[tmp1];
    }
