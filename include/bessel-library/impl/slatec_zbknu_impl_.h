/* 
    ...
    File: include/bessel-library/impl/slatec_zbknu_impl_.h
    ...
    Description:
        Slatec's ZBKNU subroutine, originally translated from Fortran to C
...
L280:
    ascle = bry[0];
    slatec_zkscl_impl_(&zdr, &zdi, fnu, n, &yr[1], &yi[1], nz, &rzr, &rzi, &ascle, tol, 
	    elim);
    inu = *n - *nz;
    if (inu <= 0) {
	return 0;
