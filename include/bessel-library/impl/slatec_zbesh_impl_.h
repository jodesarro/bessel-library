/* 
    ...
    File: include/bessel-library/impl/slatec_zbesh_impl_.h
    ...
    Description:
        Slatec's ZBESH subroutine, originally translated from Fortran to C
        with f2c, and adapted.
*/

#ifndef BESSEL_LIBRARY_SLATEC_ZBESH_IMPL_H
#define BESSEL_LIBRARY_SLATEC_ZBESH_IMPL_H
...
/* DECK ZBESH */
/* Subroutine */
static inline int slatec_zbesh_impl_(double *zr, double *zi, double *fnu, 
...
/* ----------------------------------------------------------------------- */
    ...
    goto L70;
L60:
    slatec_zuoik_impl_(&znr, &zni, fnu, kode, &c_u_2, &nn, &cyr[1], &cyi[1], &nuf, &tol, &
	    elim, &alim);
    if (nuf < 0) {
	goto L230;
    }
...
/*     YN.GE.0. .OR. M=1) */
/* ----------------------------------------------------------------------- */
    slatec_zbknu_impl_(&znr, &zni, fnu, kode, &nn, &cyr[1], &cyi[1], nz, &tol, &elim, &
	    alim);
    goto L110;
/* ----------------------------------------------------------------------- */
...
L80:
    mr = -mm;
    slatec_zacon_impl_(&znr, &zni, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw, &rl, &fnul,
	     &tol, &elim, &alim);
    if (nw < 0) {
	goto L240;
    }
...
/* ----------------------------------------------------------------------- */
    ...
    zni = -zni;
L100:
    slatec_zbunk_impl_(&znr, &zni, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw, &tol, &
	    elim, &alim);
    if (nw < 0) {
	goto L240;
    }
...
/* ----------------------------------------------------------------------- */
    d_u_1 = -fmm;
    sgn = f2c_d_sign_impl_(&hpi, &d_u_1);
/* ----------------------------------------------------------------------- */
...
#endif /* BESSEL_LIBRARY_SLATEC_ZBESH_IMPL_H */