/*
  ...
  Description: Slatec's ZBESH subroutine, originally translated from Fortran to
...
static inline int slatec_zbesh_impl_(double *zr, double *zi, double *fnu,
  ...
  *nz = 0;
  ...
  goto L70;
L60:
  slatec_zuoik_impl_(&znr, &zni, fnu, kode, &c_u_2, &nn, &cyr[1], &cyi[1], &nuf,
                     &tol, &elim, &alim);
  if (nuf < 0) {
    goto L230;
  }
...
L70:
  ...
  /*     YN.GE.0. .OR. M=1) */
  /* ----------------------------------------------------------------------- */
  slatec_zbknu_impl_(&znr, &zni, fnu, kode, &nn, &cyr[1], &cyi[1], nz, &tol,
                     &elim, &alim);
  goto L110;
/* ----------------------------------------------------------------------- */
...
L80:
  mr = -mm;
  slatec_zacon_impl_(&znr, &zni, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw,
...
L90:
  ...
  zni = -zni;
L100:
  slatec_zbunk_impl_(&znr, &zni, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw,
                     &tol, &elim, &alim);
  if (nw < 0) {
    goto L240;
  }
