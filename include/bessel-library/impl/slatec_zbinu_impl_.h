/* 
    ...
    Description:
        Slatec's ZBINU subroutine, originally translated from Fortran to C
...
/*     POWER SERIES */
/* ----------------------------------------------------------------------- */
    slatec_zseri_impl_(zr, zi, fnu, kode, &nn, &cyr[1], &cyi[1], &nw, tol, elim, alim);
    inw = abs(nw);
    *nz += inw;
    nn -= inw;
    if (nn == 0) {
...
/* ----------------------------------------------------------------------- */
L30:
    slatec_zasyi_impl_(zr, zi, fnu, kode, &nn, &cyr[1], &cyi[1], &nw, rl, tol, elim, alim)
	    ;
    if (nw < 0) {
	goto L130;
    }
...
/* ----------------------------------------------------------------------- */
    slatec_zuoik_impl_(zr, zi, fnu, kode, &c_u_1, &nn, &cyr[1], &cyi[1], &nw, tol, elim, 
	    alim);
    if (nw < 0) {
	goto L130;
    }
...
/* ----------------------------------------------------------------------- */
    slatec_zuoik_impl_(zr, zi, fnu, kode, &c_u_2, &c_u_2, cwr, cwi, &nw, tol, elim, alim);
    if (nw >= 0) {
	goto L100;
    }
    *nz = nn;
...
L100:
    ...
    slatec_zwrsk_impl_(zr, zi, fnu, kode, &nn, &cyr[1], &cyi[1], &nw, cwr, cwi, tol, elim,
	     alim);
    if (nw < 0) {
	goto L130;
    }
...
/* ----------------------------------------------------------------------- */
    nui = (int) (*fnul - dfnu + 1);
    nui = max_impl_(nui,0);
    slatec_zbuni_impl_(zr, zi, fnu, kode, &nn, &cyr[1], &cyi[1], &nw, &nui, &nlast, fnul, 
	    tol, elim, alim);
    if (nw < 0) {
	goto L130;
    }
