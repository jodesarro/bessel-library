/*
  ...
  File: include/bessel-library/impl/slatec_zunik_impl_.h
  Language standards: C99
  References: include/bessel-library/references.txt
  License: include/bessel-library/license.txt
  Repository: <https://github.com/jodesarro/bessel-library>

  Description: Slatec's ZUNIK subroutine, originally translated from Fortran to
  C with f2c, and adapted.
*/

#ifndef BESSEL_LIBRARY_SLATEC_ZUNIK_IMPL_H
#define BESSEL_LIBRARY_SLATEC_ZUNIK_IMPL_H
...
/* DECK ZUNIK */
/* Subroutine */
static inline int slatec_zunik_impl_(double *zrr, double *zri, double *fnu,
                                     int *ikflg, int *ipmtr, double *tol,
                                     int *init, double *phir, double *phii,
                                     double *zeta1r, double *zeta1i,
                                     double *zeta2r, double *zeta2i,
                                     double *sumr, double *sumi, double *cwrkr,
                                     double *cwrki) {
  /* Initialized data */
  static int c_u_1 = 1;
  ...
  /* Local variables */
  static int i_u_, j, k, l;
  static double ac, si, ti, sr, tr, t2i, t2r, rfn, sri, sti, zni, srr, str, znr;
  static int idum;
  ...
  /* ----------------------------------------------------------------------- */
  test = fortran_d1mach_impl_(&c_u_1) * 1e3;
  ac = *fnu * test;
  if (fabs(*zrr) > ac || fabs(*zri) > ac) {
    goto L15;
  }
  *zeta1r = (d_u_1 = log(test), fabs(d_u_1)) * 2. + *fnu;
  *zeta1i = 0.;
  *zeta2r = *fnu;
  *zeta2i = 0.;
  *phir = 1.;
...
L15:
  ...
  slatec_zsqrt_impl_(&sr, &si, &srr, &sri);
  str = coner + srr;
  sti = conei + sri;
  slatec_zdiv_impl_(&str, &sti, &tr, &ti, &znr, &zni);
  slatec_zlog_impl_(&znr, &zni, &str, &sti, &idum);
  *zeta1r = *fnu * str;
  *zeta1i = *fnu * sti;
  *zeta2r = *fnu * srr;
  *zeta2i = *fnu * sri;
  slatec_zdiv_impl_(&coner, &conei, &srr, &sri, &tr, &ti);
  srr = tr * rfn;
  sri = ti * rfn;
  slatec_zsqrt_impl_(&srr, &sri, &cwrkr[16], &cwrki[16]);
  *phir = cwrkr[16] * con[*ikflg - 1];
  *phii = cwrki[16] * con[*ikflg - 1];
  if (*ipmtr != 0) {
    return 0;
  }
  slatec_zdiv_impl_(&coner, &conei, &sr, &si, &t2r, &t2i);
  cwrkr[1] = coner;
  cwrki[1] = conei;
  crfnr = coner;
  crfni = conei;
  ...
  for (k = 2; k <= 15; ++k) {
    ...
    ac *= rfn;
    test = (d_u_1 = cwrkr[k], fabs(d_u_1)) + (d_u_2 = cwrki[k], fabs(d_u_2));
    if (ac < *tol && test < *tol) {
      goto L30;
    }
    /* L20: */
...
#endif /* BESSEL_LIBRARY_SLATEC_ZUNIK_IMPL_H */