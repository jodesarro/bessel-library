/*
  ...
  Description: Slatec's ZBESK subroutine, originally translated from Fortran to
...
static inline int slatec_zbesk_impl_(double *zr, double *zi, double *fnu,
  ...
  *nz = 0;
  ...
  goto L60;
L50:
  slatec_zuoik_impl_(zr, zi, fnu, kode, &c_u_2, &nn, &cyr[1], &cyi[1], &nuf,
                     &tol, &elim, &alim);
  if (nuf < 0) {
    goto L180;
  }
...
L60:
  ...
  /* ----------------------------------------------------------------------- */
  slatec_zbknu_impl_(zr, zi, fnu, kode, &nn, &cyr[1], &cyi[1], &nw, &tol, &elim,
                     &alim);
  if (nw < 0) {
    goto L200;
  }
...
L70:
  ...
  slatec_zacon_impl_(zr, zi, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw, &rl,
                     &fnul, &tol, &elim, &alim);
  if (nw < 0) {
    goto L200;
  }
...
L90:
  slatec_zbunk_impl_(zr, zi, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw, &tol,
                     &elim, &alim);
  if (nw < 0) {
    goto L200;
  }
