/* 
    ...
    File: include/bessel-library/impl/slatec_zuni1_impl_.h
    ...
    Description:
        Slatec's ZUNI1 subroutine, originally translated from Fortran to C
...
/* ----------------------------------------------------------------------- */
    fn = fmax_impl_(*fnu,1.);
    init = 0;
    slatec_zunik_impl_(zr, zi, &fn, &c_u_1, &c_u_1, tol, &init, &phir, &phii, &zeta1r, &
...
L30:
	...
	fn = *fnu + (nd - i_u_);
	init = 0;
	slatec_zunik_impl_(zr, zi, &fn, &c_u_1, &c_u_0, tol, &init, &phir, &phii, &zeta1r, &
		zeta1i, &zeta2r, &zeta2i, &sumr, &sumi, cwrkr, cwrki);
	if (*kode == 1) {
	    goto L40;
	}
