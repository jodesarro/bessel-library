/* 
    ...
    File: include/bessel-library/impl/slatec_zbesy_impl_.h
    ...
    Description:
        Slatec's ZBESY subroutine, originally translated from Fortran to C
        with f2c, and adapted.
*/

#ifndef BESSEL_LIBRARY_SLATEC_ZBESY_IMPL_H
#define BESSEL_LIBRARY_SLATEC_ZBESY_IMPL_H
...
/* DECK ZBESY */
/* Subroutine */
static inline int slatec_zbesy_impl_(double *zr, double *zi, double *fnu, 
...
/* ***FIRST EXECUTABLE STATEMENT  ZBESY */
	...
	*ierr = 1;
    ...
    hcii = .5;
    slatec_zbesh_impl_(zr, zi, fnu, kode, &c_u_1, n, &cyr[1], &cyi[1], &nz1, ierr);
    if (*ierr != 0 && *ierr != 3) {
	goto L170;
    }
    slatec_zbesh_impl_(zr, zi, fnu, kode, &c_u_2, n, &cwrkr[1], &cwrki[1], &nz2, ierr);
    if (*ierr != 0 && *ierr != 3) {
	goto L170;
    }
    *nz = min_impl_(nz1,nz2);
    if (*kode == 2) {
	goto L60;
    }
    i_u_1 = *n;
    for (i_u_ = 1; i_u_ <= i_u_1; ++i_u_) {
	str = cwrkr[i_u_] - cyr[i_u_];
	sti = cwrki[i_u_] - cyi[i_u_];
	cyr[i_u_] = -sti * hcii;
	cyi[i_u_] = str * hcii;
/* L50: */
    }
...
L70:
    *nz = 0;
    rtol = 1. / tol;
    ascle = fortran_d1mach_impl_(&c_u_1) * rtol * 1e3;
    i_u_1 = *n;
    for (i_u_ = 1; i_u_ <= i_u_1; ++i_u_) {
...
/*       CYI(I) = STR*HCII */
	...
	atol = 1.;
/* Computing MAX */
	d_u_1 = fabs(aa), d_u_2 = fabs(bb);
	if (fmax_impl_(d_u_1,d_u_2) > ascle) {
	    goto L75;
	}
	aa *= rtol;
	bb *= rtol;
...
#endif /* BESSEL_LIBRARY_SLATEC_ZBESY_IMPL_H */