/*
  ...
  Description: Slatec's ZBESY subroutine, originally translated from Fortran to
...
static inline int slatec_zbesy_impl_(double *zr, double *zi, double *fnu,
  ...
  *nz = 0;
  ...
  hcii = .5;
  slatec_zbesh_impl_(zr, zi, fnu, kode, &c_u_1, n, &cyr[1], &cyi[1], &nz1,
                     ierr);
  if (*ierr != 0 && *ierr != 3) {
    goto L170;
  }
  slatec_zbesh_impl_(zr, zi, fnu, kode, &c_u_2, n, &cwrkr[1], &cwrki[1], &nz2,
                     ierr);
  if (*ierr != 0 && *ierr != 3) {
    goto L170;
  }
