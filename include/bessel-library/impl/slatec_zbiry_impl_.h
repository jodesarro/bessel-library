/*
  ...
  Description: Slatec's ZBIRY subroutine, originally translated from Fortran to
...
L110:
  ...
  /* ----------------------------------------------------------------------- */
  slatec_zbinu_impl_(&ztar, &ztai, &fnu, kode, &c_u_1, &cyr[1], &cyi[1], &nz,
  ...
  fnu = (2. - fid) / 3.;
  slatec_zbinu_impl_(&ztar, &ztai, &fnu, kode, &c_u_2, &cyr[1], &cyi[1], &nz,
                     &rl, &fnul, &tol, &elim, &alim);
  cyr[1] *= z3r;
  cyi[1] *= z3r;
  cyr[2] *= z3r;
