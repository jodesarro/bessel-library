    ...
    File: include/bessel-library/impl/slatec_zairy_impl_.h
    ...
    Description:
        Slatec's ZAIRY subroutine, originally translated from Fortran to C
        with f2c, and adapted.
*/

#ifndef BESSEL_LIBRARY_SLATEC_ZAIRY_IMPL_H
...
/* DECK ZAIRY */
/* Subroutine */
static inline int slatec_zairy_impl_(double *zr, double *zi, int *id, 
	...
	*ierr)
{
    ...
    static double rl;
    static int mr;
    static double s1i, az3, s2i, s1r, s2r, z3i, z3r, dig, fid, cyi[3], 
	    r1m5, fnu, cyr[3], tol, sti, ptr, str, sfac, alim, elim, alaz, 
	    csqi;
...
L40:
    ...
    *aii = s1i * c1 - c2 * (*zr * s2i + *zi * s2r);
    ...
    slatec_zexp_impl_(&ztar, &ztai, &str, &sti);
    ptr = *air * str - *aii * sti;
    *aii = *air * sti + *aii * str;
    *air = ptr;
    return 0;
...
/* ----------------------------------------------------------------------- */
    ...
    slatec_zacai_impl_(&ztar, &ztai, &fnu, kode, &mr, &c_u_1, &cyr[1], &cyi[1], &nn, &rl, &tol, &
	    elim, &alim);
    if (nn < 0) {
	goto L280;
    }
...
L120:
    slatec_zbknu_impl_(&ztar, &ztai, &fnu, kode, &c_u_1, &cyr[1], &cyi[1], nz, &tol, &elim, &alim);
L130:
    s1r = cyr[1] * coef;
    s1i = cyi[1] * coef;
    if (iflag != 0) {
...
L160:
    ...
    *aii = s1i / sfac;
    return 0;
L170:
    aa = fortran_d1mach_impl_(&c_u_1) * 1e3;
    s1r = zeror;
    s1i = zeroi;
    if (*id == 1) {
	goto L190;
...
#endif /* BESSEL_LIBRARY_SLATEC_ZAIRY_IMPL_H */
