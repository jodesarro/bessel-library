/*
  ...
  Description: Slatec's ZAIRY subroutine, originally translated from Fortran to
...
/* DECK ZAIRY */
/* Subroutine */
static inline int slatec_zairy_impl_(double *zr, double *zi, int *id, int *kode,
...
L100:
  ...
  slatec_zacai_impl_(&ztar, &ztai, &fnu, kode, &mr, &c_u_1, &cyr[1], &cyi[1],
                     &nn, &rl, &tol, &elim, &alim);
  if (nn < 0) {
    goto L280;
  }
...
L120:
  slatec_zbknu_impl_(&ztar, &ztai, &fnu, kode, &c_u_1, &cyr[1], &cyi[1], nz,
                     &tol, &elim, &alim);
L130:
  s1r = cyr[1] * coef;
  s1i = cyi[1] * coef;
