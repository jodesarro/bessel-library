/* 
    ...
    File: include/bessel-library/impl/slatec_zacai_impl_.h
    ...
    Description:
        Slatec's ZACAI subroutine, originally translated from Fortran to C
...
/* ----------------------------------------------------------------------- */
    slatec_zseri_impl_(&znr, &zni, fnu, kode, &nn, &yr[1], &yi[1], &nw, tol, elim, alim);
    goto L40;
L20:
    if (az < *rl) {
	goto L30;
...
/* ----------------------------------------------------------------------- */
    slatec_zasyi_impl_(&znr, &zni, fnu, kode, &nn, &yr[1], &yi[1], &nw, rl, tol, elim, 
	    alim);
    if (nw < 0) {
	goto L80;
    }
...
/* ----------------------------------------------------------------------- */
    slatec_zbknu_impl_(&znr, &zni, fnu, kode, &c_u_1, cyr, cyi, &nw, tol, elim, alim);
    if (nw != 0) {
	goto L80;
    }
    fmr = (double) (*mr);
...
L60:
    ...
    slatec_zs1s2_impl_(&znr, &zni, &c1r, &c1i, &c2r, &c2i, &nw, &ascle, alim, &iuf);
    *nz += nw;
L70:
