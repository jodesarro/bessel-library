/* 
    ...
    File: include/bessel-library/impl/slatec_zacon_impl_.h
    ...
    Description:
        Slatec's ZACON subroutine, originally translated from Fortran to C
...
/* ***FIRST EXECUTABLE STATEMENT  ZACON */
    *nz = 0;
    ...
    zni = -(*zi);
    nn = *n;
    slatec_zbinu_impl_(&znr, &zni, fnu, kode, &nn, &yr[1], &yi[1], &nw, rl, fnul, tol, 
	    elim, alim);
    if (nw < 0) {
	goto L90;
    }
...
/* ----------------------------------------------------------------------- */
    nn = min_impl_(2,*n);
    slatec_zbknu_impl_(&znr, &zni, fnu, kode, &nn, cyr, cyi, &nw, tol, elim, alim);
    if (nw != 0) {
	goto L90;
    }
    s1r = cyr[0];
