/*
  ...
  {I_nu(z), I_(nu+1)(z), ..., I_(nu+n-1)(z)}, for also negative orders, by means
...
static inline void cyl_i_full_seq_impl_(double nu, int n, dcomplex z,
  ...
  if (nu >= 0.0) {
    ...
    /* Compute zbesi */
    slatec_zbesi_impl_(&x, &y, &fnu, &kode, &n, &cir[0], &cii[0], &nz, &ierr);
    ...
    if (fabs(floor(fnu) - fnu) < DBL_EPSILON) {
      ...
      slatec_zbesi_impl_(&x, &y, &fnu_m, &kode, &n, &cir[0], &cii[0], &nz,
                         &ierr);
      slatec_flags_zbesi_impl_(ierr, nz);
      ...
      /* Compute zbesk */
      slatec_zbesk_impl_(&x, &y, &fnu_m, &kodek, &n, &ckr[0], &cki[0], &nz,
                         &ierr);
    ...
    if (fabs(floor(fnu) - fnu) < DBL_EPSILON) {
      ...
      /* Compute zbesi */
      slatec_zbesi_impl_(&x, &y, &fnu_m, &kode, &n_m, &cir_m[0], &cii_m[0], &nz,
                         &ierr);
      slatec_flags_zbesi_impl_(ierr, nz);
    ...
    } else if (fabs(floor(nu_m) - nu_m) >= DBL_EPSILON && cabs(z) == 0.0) {
      ...
      /* Compute zbesk */
      slatec_zbesk_impl_(&x, &y, &fnu_m, &kodek, &n_m, &ckr_m[0], &cki_m[0],
                         &nz, &ierr);
      slatec_flags_zbesk_impl_(ierr, nz);
    ...
    slatec_zbesi_impl_(&x, &y, &fnu_p, &kode, &n_p, &cir_p[0], &cii_p[0], &nz,
                       &ierr);
    slatec_flags_zbesi_impl_(ierr, nz);
