/* 
    ...
    File: include/bessel-library/impl/slatec_zbuni_impl_.h
    ...
    Description:
        Slatec's ZBUNI subroutine, originally translated from Fortran to C
...
/* ----------------------------------------------------------------------- */
    slatec_zuni1_impl_(zr, zi, &gnu, kode, &c_u_2, cyr, cyi, &nw, nlast, fnul, tol, elim, 
	    alim);
    goto L20;
L10:
/* ----------------------------------------------------------------------- */
...
/*     AND HPI=PI/2 */
/* ----------------------------------------------------------------------- */
    slatec_zuni2_impl_(zr, zi, &gnu, kode, &c_u_2, cyr, cyi, &nw, nlast, fnul, tol, elim, 
	    alim);
L20:
    if (nw < 0) {
	goto L50;
...
/*     -PI/3.LE.ARG(Z).LE.PI/3 */
/* ----------------------------------------------------------------------- */
    slatec_zuni1_impl_(zr, zi, fnu, kode, n, &yr[1], &yi[1], &nw, nlast, fnul, tol, elim, 
	    alim);
    goto L80;
L70:
/* ----------------------------------------------------------------------- */
...
/*     AND HPI=PI/2 */
/* ----------------------------------------------------------------------- */
    slatec_zuni2_impl_(zr, zi, fnu, kode, n, &yr[1], &yi[1], &nw, nlast, fnul, tol, elim, 
	    alim);
L80:
    if (nw < 0) {
	goto L50;
