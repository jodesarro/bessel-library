/* 
    ...
    File: include/bessel-library/impl/slatec_zunk2_impl_.h
    ...
    Description:
        Slatec's ZUNK2 subroutine, originally translated from Fortran to C
...
/* ----------------------------------------------------------------------- */
	j = 3 - j;
	fn = *fnu + (i_u_ - 1);
	slatec_zunhj_impl_(&znr, &zni, &fn, &c_u_0, tol, &phir[j - 1], &phii[j - 1], &argr[
...
/* ----------------------------------------------------------------------- */
    ...
    slatec_zunhj_impl_(&znr, &zni, &fn, &ipard, tol, &phidr, &phidi, &argdr, &argdi, &
...
L175:
	...
	slatec_zunhj_impl_(&znr, &zni, &fn, &c_u_0, tol, &phidr, &phidi, &argdr, &argdi, &
		zet1dr, &zet1di, &zet2dr, &zet2di, &asumdr, &asumdi, &bsumdr, 
		&bsumdi);
L210:
	if (*kode == 1) {
...
L240:
	...
	slatec_zairy_impl_(&argdr, &argdi, &c_u_1, &c_u_2, &dair, &daii, &ndai, &idum);
...
/* ----------------------------------------------------------------------- */
	...
	slatec_zs1s2_impl_(&zrr, &zri, &s1r, &s1i, &s2r, &s2i, &nw, &asc, alim, &iuf);
	*nz += nw;
L270:
...
/* ----------------------------------------------------------------------- */
	...
	slatec_zs1s2_impl_(&zrr, &zri, &c1r, &c1i, &c2r, &c2i, &nw, &asc, alim, &iuf);
	*nz += nw;
L300:
